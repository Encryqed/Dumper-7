...
StructInfoHandle::StructInfoHandle( const StructInfo& InInfo ) : Info( &InInfo )
{
}
...
void StructManager::InitAlignmentsAndNames( )
{
	...
	AllStructs.reserve( ObjectCount / 4 ); // More accurate reserve based on typical struct ratio
...
void StructManager::InitSizesAndIsFinal( )
{
	...
	for ( const auto& [Index, Info] : StructInfoOverrides )
	{
		...
		if ( NewOrExistingInfo.Size == 0x0 || NewOrExistingInfo.Size == INT_MAX )
