...
{
	// Cache the name index to avoid repeated hash lookups
	const auto NameIdx = NamePair.first;
	const auto bWasInserted = NamePair.second;
	...
	static auto AddCollidingName = [ ] ( const NameContainer& SearchNames, NameContainer* OutTargetNames, HashStringTableIndex NameIdx, ECollisionType CurrentType, bool bIsSuper ) -> bool
		{
			...
			// Use forward iterator and reserve memory to reduce allocations
			OutTargetNames->reserve(OutTargetNames->size() + 1);
			
			for ( const auto& ExistingName : SearchNames )
			{
