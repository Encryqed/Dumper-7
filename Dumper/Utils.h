...
public:
	...
	inline MemAddress FindFunctionEnd(uint32 Range = 0xFFFF)
	{
		if (!Address)
			return nullptr;
	...
	inline MemAddress RelativePattern(const char* Pattern, int32_t Range, int32_t Relative = 0)
	{
		if (!Address)
			return nullptr;
	...
	*	Function considers all E8 bytes that would make for a valid call (to address within process-bounds).
	* 
	...
	*/
	inline MemAddress GetRipRelativeCalledFunction(int32_t OneBasedFuncIndex, bool(*IsWantedTarget)(MemAddress CalledAddr) = nullptr)
	{
